...
class Stock:
    ...
    def add_data(self, date, close, volume):
    ...
    def buy(self, amount):
        if amount <= self._shares:
            ...
            print("Insufficient shares to buy.")
...
def main():
    ...
    try:
        ...
        error_count = error_count+1
        error_list.append(
            "Stock symbol change allowed. Stock symbol changes should not be allowed.")
    except:
    ...
    try:
        ...
        error_count = error_count+1
        error_list.append(
            "Stock shares change allowed. Change in shares should be done through buy() or sell().")
    except:
    ...
    try:
        dayData = DailyData(datetime.strptime(
            "1/1/20", "%m/%d/%y"), float(14.50), float(100000))
        testStock.add_data(dayData.date, dayData.close, dayData.volume)
